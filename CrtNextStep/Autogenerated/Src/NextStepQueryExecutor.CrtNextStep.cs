namespace Terrasoft.Configuration.NextSteps
{
	...
	internal class NextStepQueryExecutor: IEntityQueryExecutor
	{
		...
		private void AddOrUpdateNextStepModels(List<NextStepModel> collection, string entityName, Guid entityId) {
			...
			var notProcesedElementsIds = collection.Where(e => e.ProcessElementId.IsNotEmpty())
				.Select(e => e.ProcessElementId).ToList();
			var elementsIds = notProcesedElementsIds.Select(id => id.ToString()).ToList();
			foreach (var executor in executors) {
				try {
					foreach (var nextStepModel in executor.GetNextSteps(entityName, entityId, elementsIds)) {
						...
						notProcesedElementsIds.Remove(nextStepModel.ProcessElementId);
			...
			if (Features.GetIsEnabled("UseDefNextStepsActivityTaskSchema") && notProcesedElementsIds.Any()) {
				collection.Where(ns => notProcesedElementsIds.Contains(ns.ProcessElementId) && ns.EntityName == "Activity")
					.ForEach(ns => ns.AdditionalInfo = AddOrReplaceAdditionalParam(ProcessElementExtraDataKeys.UserTaskEntitySchemaNameKey,
					ns.AdditionalInfo, $"{{\"{ProcessElementExtraDataKeys.UserTaskEntitySchemaNameKey}\":\"Activity\"}}"));
			}
		}
