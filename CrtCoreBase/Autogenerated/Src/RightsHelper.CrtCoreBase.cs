	...
	public class RightsHelper
	{
		...
		public void AddNewRecordRights(RecordRight[] recordRights, string rightsSchemaName, Record record) {
			foreach(var item in recordRights) {
				...
				bool useDenyRecordRights = _userConnection.EntitySchemaManager.GetInstanceByName(record.entitySchemaName).UseDenyRecordRights;
				if (useDenyRecordRights && item.Position >= 0) {
		...
		public  List<RecordRight> GetRecordRights(string tableName, string recordId) {
			...
			if (Features.GetIsEnabled("HideDenyRecordRights")) {
				sysRecordRightsSelect.And(tableName, "RightLevel").IsGreater(Column.Parameter(0));
			}
			using (var dbExecutor = _userConnection.EnsureDBConnection()) {
