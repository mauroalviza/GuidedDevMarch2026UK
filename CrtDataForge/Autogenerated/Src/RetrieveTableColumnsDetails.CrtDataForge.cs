	...
	public class RetrieveTableColumnsDetailsMethodsWrapper : ProcessModel
	{
		...
		private bool RetrieveColumnsDetailsExecute(ProcessExecutingContext context) {
			...
			try
			{
			    ...
			    Set("IsSuccess", true);
			    Set("Columns", compositeColumnsObjects);
			}
			catch (Terrasoft.Common.ItemNotFoundException ex)
			{
			    // Entity schema was not found
			    Set("IsSuccess", false);
			    Set("ErrorMessage", $"Entity schema '{tableName}' was not found.");
			}
			catch (Exception ex)
			{
			    // Any other unexpected error
			    Set("IsSuccess", false);
			    Set("ErrorMessage", $"Unexpected error: {ex.Message}");
			}
			return true;
		}
