 namespace Terrasoft.Configuration.Translation
{
	...
	public class ApplyTranslationAppEventListener : AppEventListenerBase
	{
		...
		private void RestartUncompletedApplyTranslation(UserConnection userConnection) {
			...
			Logger.InfoFormat("Try scheduling to restart uncompleted apply translation process with job name {0}", jobName);
			...
			ITrigger trigger = new SimpleTriggerImpl(RestartApplyTranslationTriggerName, JobGroupName);
			var date = _schedulerWrapper.Instance.ScheduleJob(job, trigger);
			Logger.InfoFormat("Scheduled to restart uncompleted apply translation process with job name {0} at {1:yyyy-MM-ddTHH:mm:ssZ} UTC",
