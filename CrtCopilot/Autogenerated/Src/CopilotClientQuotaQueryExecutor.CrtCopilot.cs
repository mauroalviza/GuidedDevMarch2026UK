namespace Creatio.Copilot
{
	...
	public class CopilotClientQuotaQueryExecutor : BaseQueryExecutor, IEntityQueryExecutor
	{
		...
		private Entity CreateCopilotClientQuotaEntity(ClientQuotaResponse data, EntitySchemaQuery esq) {
			...
			MapCopilotClientTokensQuota(data, esq, entity);
			MapCopilotClientActionsQuota(data, esq, entity);
			return entity;
		}

		private void MapCopilotClientTokensQuota(ClientQuotaResponse data, EntitySchemaQuery esq, Entity entity) {
		...
		private void MapCopilotClientActionsQuota(ClientQuotaResponse data, EntitySchemaQuery esq, Entity entity) {
			const string actionsAvailableColumnName = "ActionsAvailable";
			long? actionsAvailableColumnValue = data.TotalActionsQuota?.Limit == -1
				? data.TotalActionsQuota?.Limit
				: data.TotalActionsQuota?.Limit - data.TotalActionsQuota?.SubscriptionUsage;
			entity.SetColumnValue("ActionsLimit", data.TotalActionsQuota?.Limit);
			entity.SetColumnValue("ActionsUsage", data.TotalActionsQuota?.SubscriptionUsage);
			entity.SetColumnValue(actionsAvailableColumnName, actionsAvailableColumnValue);
			EntitySchemaQueryColumn aggregationColumn = esq.Columns.FirstOrDefault(c =>
				c.IsAggregated && (c.ValueExpression?.Function as EntitySchemaAggregationQueryFunction)?.Expression?.Path == actionsAvailableColumnName);
			if (aggregationColumn != null) {
				if (entity.Schema.Columns.FindByColumnValueName(aggregationColumn.Name) == null) {
					entity.Schema.AddColumn("Integer", aggregationColumn.Name);
				}
				entity.ForceSetColumnValue(aggregationColumn.Name, entity.GetColumnValue(actionsAvailableColumnName));
			}
		}
