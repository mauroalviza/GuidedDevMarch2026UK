namespace Creatio.Copilot
{
	...
	internal class CopilotPromptFactory : ICopilotPromptFactory
	{
		...
		internal static readonly string[] ChatGlobalSettingsSectionPrompts = new[] {
		...
		private class ApiSystemPromptFactory : PromptFactory
		{
			...
			private const string RulesForConversationTerminationSection = "## Rules for Conversation termination";
			...
			private static IList<string> GetSectionNameOrder() {
				...
				if (Features.GetIsEnabled<GenAIFeatures.UseAgenticProcesses>()) {
					sectionNames.Add(RulesForConversationTerminationSection);
				}
				sectionNames.Add(RulesForResponseGenerationSection);
				...
				new Dictionary<string, IList<string>> {
					{
						...
						RulesForConversationTerminationSection,
						new[] {
							"When the user states that they want to stop or cancel conversation, call [CancelProcessExecutionAction] and answer according to its result.",
						}
					}, {
