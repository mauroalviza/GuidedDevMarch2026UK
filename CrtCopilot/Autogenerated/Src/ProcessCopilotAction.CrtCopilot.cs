namespace Creatio.Copilot
{
	...
	public class ProcessCopilotAction : CopilotAction
	{
		...
		public class ProcessCompletionHook : CopilotBaseCompletionHook, IBackgroundTask<HookArgs>
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="ProcessCompletionHook"/> type.
			/// </summary>
			/// <param name="copilotEngine">Copilot engine.</param>
			...
			private static void ExtractProcessInfo(IProcessInfo processInfo, ProcessExecutionErrorHookArgs errorHookArgs,
					out bool isSuccess, out string errorMessage)
			{
				ProcessStatus processStatus = processInfo.Descriptor.ProcessStatus;
				isSuccess = processStatus == ProcessStatus.Done;
				errorMessage = processStatus == ProcessStatus.Cancelled
					? "The process was cancelled."
					: errorHookArgs?.ErrorMessage;
			}
			...
			private CopilotActionExecutionResult FillResults(HookArgs args, CopilotActionDescriptor actionDescriptor) {
				...
				try {
					...
					ProcessExecutionErrorHookArgs errorHookArgs = args.ProcessHookInfo as ProcessExecutionErrorHookArgs;
					string response = GetResponse(userConnection, processInfo, actionDescriptor.Parameters,
						_responseProvider);
					ExtractProcessInfo(processInfo, errorHookArgs, out bool isSuccess, out string errorMessage);
			...
			private void HandleCompletion(HookArgs parameters) {
		...
		public ProcessCopilotAction(CopilotActionMetaItem actionMetaItem, UserConnection userConnection)
			...
			_processExecutor = ClassFactory.Get<ICopilotProcessExecutor>(
		...
		private static ILog Log => _log ?? (_log = LogManager.GetLogger("Copilot"));
		...
		protected override CopilotActionExecutionResult InternalRun(CopilotActionExecutionOptions options) {
			...
			try {
				ProcessDescriptor processResult = _processExecutor.StartProcess(
					processSchemaUId,
					values,
					(hookOptions, args, events, context) => hookOptions.WithHook<ProcessCompletionHook, ProcessCompletionHookArgs>(
						(ProcessCompletionHookArgs)args, events, context),
					hookArgs,
					hookEvents,
					resultBucket);
