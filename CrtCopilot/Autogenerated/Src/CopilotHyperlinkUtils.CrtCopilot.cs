namespace Creatio.Copilot
{
	...
	using ConfigurationSectionHelper = Terrasoft.Configuration.ConfigurationSectionHelper;
	...
	/// </summary>
	public interface ICopilotHyperlinkUtils
	{
	...
	internal class CopilotHyperlinkUtils: ICopilotHyperlinkUtils
	{
		...
		private bool IsEditPageExist(string schemaName) {
			var configurationData = _configurationSectionHelper.GetConfigurationData(_userConnection, new []{ schemaName });
			var entityStructure = configurationData?.EntitiesStructure?.FirstOrDefault().Value;
			return entityStructure?.PagesStructure?.Count > 0;
		}
		...
		private bool IsLinkValid(string link, string linkBaseUrl) {
			...
			return IsRecordAccessible(schemaName, recordId) && IsEditPageExist(schemaName);
		}
